...
def draw_map_points(user_position, points):
...
def find_nearest_stations(user_position, num_k, dock_bikes):
    points = []
    nearest_stations = []
    type = 'bikes'
    ...
    if len(points) < k_stations:
        print_error('Not enough stations available')

    draw_map_points(user_position, points)
...
def find_shortest_route( start_location, end_location):
    ...
    available_routes = []
    closest_stations = find_nearest_stations(str(start_location[0])+','+str(start_location[1]), 6, 'bikes')
...
def find_route_between_points(start_location, end_location):
    ...
    draw_route_map(start_location, end_location, shortest_route)
